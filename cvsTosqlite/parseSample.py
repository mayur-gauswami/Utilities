...
c.execute('create table testTable(id integer primary key autoincrement, name text, city text, state text, zip integer, close_date text, updated_date text)')
...
data = open("sample.csv", "r").readlines()[1:]
for entry in data:
    # Parse values
...
c.execute("select strftime('%Y', close_date), count(*) from testTable group by 1;")
years = []
failed_banks = []
for row in c:
    years.append(row[0])
